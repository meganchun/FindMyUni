	...
	public static void getData() throws IOException, HttpStatusException {
		...
		for (int num = 300; num <= 500; num++) {
			...
			try {
				...
				Element h4Class2 = doc.select("div.tabbed-subsection").get(2);
				...
				for (int i = 0; i < ul.size(); i++) {
					...
					Element test = ul.get(i);
					
					if (test.text().contains("4U") || test.text().contains("4M")) {
						...
						for (int j = 0; j < li.size(); j++) {
							...
							if (li.get(j).text().contains("4U") || li.get(j).text().contains("4M"))
							System.out.println(li.get(j).text());
						}
					}
				}
				*/
				...
				if (validUni) {
					...
					if (programName.indexOf("Engineering") != -1) {
						...
						String[] info = {programName, universityName, gradeRange, degree, address, programLink}; 
