...
public class CreateAccount extends JFrame implements KeyListener, ActionListener {
	...
	public CreateAccount() {
		...
		confirmPassword.setBounds(350, 440, 500, 65);
		...
		createAccountBtn.setBounds(350, 560, 500, 65); //set bounds
		...
		if (e.getSource() == createAccountBtn) {
			...
			if (password.getText().length() >= 6) {
				...
				if (confirmPassword.getText().equals(password.getText())) {
					...
					SecretKeySpec key = encrypt.getSecretKeySpec();
					
					byte[] encryptedUsernameByte = encrypt.encryptKey(key, username.getText());
					byte[] encryptedPasswordByte = encrypt.encryptKey(key, password.getText());
					
					System.out.println(encrypt.decrypt(key, encryptedUsernameByte));
					System.out.println(encrypt.decrypt(key, encryptedPasswordByte));
					
					User newUser = new User(encryptedUsernameByte, encryptedPasswordByte);
	...
	public void addToFile(String fileName, User user) {
		...
		try {
			...
			String info = new String(user.getUserName())+","+new String(user.getUserName());
