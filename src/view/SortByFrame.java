...
public class SortByFrame extends JFrame implements ActionListener{
	...
	String[] universityList = {"Ontario Tech University", "Queen's University", "Brock University",
							...
							"University of Guelph", "University of Toronto", "University of Waterloo",
							"University of Windsor", "Western University", "York University", "Trent University"};
	
	//Kinesiology + Nursing + Biochemistry + Biomedical +
	String[] programList = {"Engineering", "Computer Science", "Medical Sciences", 
							"Earth Sciences", "Mathematic", "Biology",
							"Chemistry", "Physics"};
...
//			}
	...
	private void addPrograms() {
		...
		for(String program: programList) {	
			...
			programs[index] = new JCheckBox(program, true);
	...
	private void doSm() {
		...
		for(JCheckBox currentCheckbox : programs) {
			
			if(values.contains(currentCheckbox.getText())) {
	...
	private void addGrades() {
		...
		//Loop for numbers 70 to 90, skipping 5 each time (for all programs the lowest they take is 70,
		//and the highest is 90, so only need to use grade ranges between there.
		for(int index = 70; index <= 85; index +=5) {
		
			
				
			grades[counter] = new JCheckBox(index + "% - " + (index+4) + "%", true);
			...
			if(index == 85) {
				grades[counter] = new JCheckBox(90 + "%+", true);
				grades[counter].setFont(FontClassJeff.regular16);
				grades[counter].setAlignmentX(Component.LEFT_ALIGNMENT);
				grades[counter].setContentAreaFilled(false);
				grades[counter].setFocusPainted(false);
